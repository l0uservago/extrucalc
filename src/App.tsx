...
import { Factory, AlertTriangle, Monitor, Layers, Ruler, List, Box, RotateCcw, Zap, ThumbsUp, TrendingDown, Scissors, Link2, BarChart3, Instagram, Activity, CheckCircle2, Clock, Flame, Truck, ClipboardList, Plus, Trash2, ChevronDown, ChevronRight, ScanLine, FileText, Download } from 'lucide-react';
import { BILLET_TYPES, INITIAL_INPUTS } from './constants';
import { CalculationInputs, Ordine, ArticoloDB, DieHistoryEntry } from './types';
...
const IndustrialInput: React.FC<{
  ...
  optional?: boolean;
  compact?: boolean;
}> = ({ label, name, value, onChange, highlight, customColor, helper, placeholder = "0", optional, compact }) => {
  const [inputValue, setInputValue] = useState<string>(value === 0 && optional ? '' : value.toString());
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      <label className={`font-black text-slate-500 uppercase flex items-center justify-between ${compact ? 'text-[8px] mb-0.5' : 'text-[9px] tracking-widest ml-1'}`}>
        {label}
        {optional && <span className="text-[7px] text-slate-600 bg-slate-800/50 px-1 py-0.5 rounded normal-case tracking-normal">Opz.</span>}
      </label>
      <input
        type="number"
        step="any"
...
const ResultCard: React.FC<{
  ...
  <div className={`p-5 rounded-xl border transition-all duration-300 ${highlight
...
const DieHistoryModal: React.FC<{
  ...
  onClose: () => void;
  onSave: (billette: number, scarto: number | null, turno: 1 | 2 | 3 | null, note: string | null, operatore: string | null) => Promise<void>;
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    await onSave(billette, scarto ? parseFloat(scarto) : null, turno, note || null, operatore || null);
    setIsSaving(false);
    setBillette(0);
    setScarto('');
    setTurno(null);
  ...
  const totalBillette = history.reduce((sum, entry) => sum + entry.billetteEstruse, 0);
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-slate-800/50 bg-slate-800/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black uppercase tracking-widest text-white leading-none mb-1">Storico Matrice</h3>
              <p className="text-[10px] font-bold text-slate-400">Art. {articolo} <span className="text-indigo-400">/ Barra {numeroBarra}</span></p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          <div className="flex items-center gap-4 p-4 bg-slate-950/50 rounded-xl border border-slate-800/50">
            ...
            <div>
              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-0.5">Vita Totale Matrice</span>
              <span className="text-2xl font-black text-white mono">{totalBillette} <span className="text-sm text-slate-500">billette passate</span></span>
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Aggiungi Sessione</h4>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-950/50 rounded-lg p-2 border border-slate-800/50">
                <label className="text-[8px] font-black uppercase text-slate-500 px-1 mb-1 block">Billette Estruse</label>
                <input type="number" value={billette === 0 ? '' : billette} onChange={e => setBillette(parseInt(e.target.value) || 0)} placeholder="Es. 80" className="w-full bg-transparent text-sm font-black text-white mono outline-none px-1" />
              </div>
              <div className="bg-slate-950/50 rounded-lg p-2 border border-slate-800/50">
                <label className="text-[8px] font-black uppercase text-slate-500 px-1 mb-1 block">Scarto Media Reale (mm)</label>
                <input type="number" value={scarto} onChange={e => setScarto(e.target.value)} placeholder="Es. 800" className="w-full bg-transparent text-sm font-black text-amber-400 mono outline-none px-1" />
              </div>
            </div>

            <div className="bg-slate-950/50 rounded-lg p-2 border border-slate-800/50">
              <label className="text-[8px] font-black uppercase text-slate-500 px-1 mb-1 block">Turno Produzione</label>
              <div className="flex gap-2">
                {[
                  ...
                  >
                    <span className="block font-black uppercase">{t.label}</span>
                    <span className="text-[8px] opacity-70 font-normal">{t.time}</span>
                  </button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-950/50 rounded-lg p-2 border border-slate-800/50 col-span-2 sm:col-span-1">
                <label className="text-[8px] font-black uppercase text-slate-500 px-1 mb-1 block">Operatore</label>
                <input type="text" value={operatore} onChange={e => setOperatore(e.target.value)} placeholder="Nome" className="w-full bg-transparent text-sm font-black text-white outline-none px-1" />
              </div>
              <div className="bg-slate-950/50 rounded-lg p-2 border border-slate-800/50 col-span-2 sm:col-span-1">
                <label className="text-[8px] font-black uppercase text-slate-500 px-1 mb-1 block">Note / Problemi</label>
                <textarea rows={2} value={note} onChange={e => setNote(e.target.value)} placeholder="Es. Chiude dx" className="w-full bg-transparent text-sm font-bold text-slate-300 outline-none px-1 resize-none h-16" />
              </div>
            </div>
            <button
              onClick={handleSave}
            ...
            >
              {isSaving ? 'Salvataggio...' : <><Plus className="w-3.5 h-3.5" /> Registra Sessione</>}
            </button>
          </div>

          <div className="space-y-3 pt-4 border-t border-slate-800/50">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Storico Lavorazioni</h4>
            {history.length === 0 ? (
              <p className="text-[10px] font-bold text-slate-600 text-center py-6 italic">Nessuna sessione registrata.</p>
            ) : (
              <div className="space-y-2">
                {history.map((h) => (
                  <div key={h.id} className="bg-slate-950/30 rounded-lg p-3 text-sm flex flex-col sm:flex-row sm:items-center justify-between border border-slate-800/30 gap-2">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black text-indigo-400 bg-indigo-500/10 px-2 py-1 rounded">+{h.billetteEstruse} Bill.</span>
                      {h.scartoReale && <span className="text-[9px] font-bold text-amber-500/80">Scarto: ~{h.scartoReale}mm</span>}
                      {h.turno && <span className="text-[9px] font-bold text-slate-300 bg-slate-800/50 px-1.5 py-0.5 rounded shadow-sm border border-slate-700/50">{h.turno}° Turno</span>}
                    </div>
                    <div className="flex flex-col sm:items-end w-full sm:w-auto overflow-hidden">
                      <span className="text-[9px] font-bold text-slate-400">{new Date(h.dataCreazione).toLocaleDateString('it-IT')} {h.operatore ? ` — ${h.operatore}` : ''}</span>
                      {h.note && <span className="text-[10px] text-slate-300 italic truncate w-full sm:max-w-[200px]" title={h.note}>"{h.note}"</span>}
                    </div>
                  </div>
                ))}
              </div>
...
const App: React.FC = () => {
  ...
  const [logNelForno, setLogNelForno] = useState<number[]>([0, 0, 0, 0, 0]);
  const [editingSlot, setEditingSlot] = useState<number | null>(null);
  const [logCaricatore, setLogCaricatore] = useState<number[]>(Array(12).fill(0));
  const [editingSlotCaricatore, setEditingSlotCaricatore] = useState<number | null>(null);
  const [currentView, setCurrentView] = useState<'calcoli' | 'gestione-log' | 'ordini' | 'archivio' | 'report'>('calcoli');

  // Report filters
  const [reportFilterDate, setReportFilterDate] = useState<string>('');
  const [reportFilterMonth, setReportFilterMonth] = useState<number>(new Date().getMonth() + 1);
  ...
  const [reportFilterMode, setReportFilterMode] = useState<'day' | 'month'>('month');
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);
  const [ordini, setOrdini] = useState<Ordine[]>([]);
  const [expandedOrdine, setExpandedOrdine] = useState<string | null>(null);
  const [barcodeInput, setBarcodeInput] = useState('');
  const [barcodeFeedback, setBarcodeFeedback] = useState<'idle' | 'success' | 'error'>('idle');
  ...
  const [activeDieModal, setActiveDieModal] = useState<{ articolo: string, numeroBarra: string } | null>(null);
  ...
  const processBarcodeString = (code: string) => {
    ...
    const dbMatch = dbArticoli.find(a => a.articolo.toLowerCase() === searchArticolo.toLowerCase());
  ...
  const handleSaveDieHistory = async (articolo: string, numeroBarra: string, billette: number, scarto: number | null, turno: 1 | 2 | 3 | null, note: string | null, operatore: string | null) => {
    const payload = {
  ...
  const computeOrdineResults = (ord: Ordine) => {
    ...
    if (!cut || cut <= 0) {
      ...
      const maxK = (maxUtileNetto > 0 && ord.lunghezzaBarra > 0) ? Math.floor(maxUtileNetto / ord.lunghezzaBarra) : 0;

      if (ord.abilitaGiunta) {
        cut = MAX_BILLETTA;
      } else if (maxK > 0) {
        const optimalUtileNetto = maxK * ord.lunghezzaBarra;
        const optimalTirata = optimalUtileNetto + ord.scartoTesta + ord.scartoCoda;
      ...
      const matchingOpt = BILLET_TYPES.find(b => b.id === logId)?.options.find(o => Math.abs(o.length - cut) <= 10);
      if (matchingOpt) {
    ...
    const bpbBase = (utileNetto > 0 && ord.lunghezzaBarra > 0)
    ...
    if (inputs.abilitaGiunta && bpbBase > 0 && ord.lunghezzaBarra > 0) {
      const scartoSingolo = utileNetto - (Math.floor(utileNetto / ord.lunghezzaBarra) * ord.lunghezzaBarra);
    ...
    const billette = bpbEffettivo > 0 ? Math.ceil(barreConSfrido / bpbEffettivo) : 0;

    const option = log?.options.find(o => o.length === cut);
    const pezziPerLog = option ? option.pieces : (cut > 0 && logLen >= cut ? Math.floor(logLen / cut) : 0);
    ...
    const logRecommendations = BILLET_TYPES.flatMap(logOpt =>
      logOpt.options.map(opt => {
        ...
        const optUtileNetto = Math.max(0, optTirata - ord.scartoTesta - ord.scartoCoda);
        const optBpb = (optUtileNetto > 0 && ord.lunghezzaBarra > 0)
          ? Math.floor(optUtileNetto / ord.lunghezzaBarra) * ord.numeroLuci
          : 0;
        const optBillette = optBpb > 0 ? Math.ceil(barreConSfrido / optBpb) : Infinity;
        const optBillPerLog = opt.pieces;
        const optLogs = optBillPerLog > 0 ? optBillette / optBillPerLog : Infinity;
        const optScarto = logOpt.length - (opt.pieces * optCut);
        const inRange = optCut >= MIN_BILLETTA && optCut <= MAX_BILLETTA &&
          optTirata >= MIN_TIRATA && optTirata <= MAX_TIRATA;

        let finalBpb = optBpb;
        if (inputs.abilitaGiunta) {
          const scNum = optUtileNetto - (Math.floor(optUtileNetto / ord.lunghezzaBarra) * ord.lunghezzaBarra);
          const geNum = Math.floor((scNum * 2) / ord.lunghezzaBarra) * ord.numeroLuci;
          finalBpb = optBpb + geNum / 2;
        }

        return {
          logId: logOpt.id, logName: logOpt.name, logLen: logOpt.length,
          cut: optCut, pieces: opt.pieces,
          tirata: optTirata, bpb: finalBpb,
          billette: optBillette, logsNeeded: optLogs, scarto: optScarto, inRange
        };
      })
    ...
    return {
      tirata, bpb: bpbBase, bpbEffettivo, giuntaExtra, barreConSfrido, billette,
      logsDecimali, logLen, logName: log?.name || '—', cut, ratio,
      scarto: scartoResiduoLog, isAutoOptimized, isStandardMatched, logId, recommendation: bestRec
    };
  };

  const ordiniResults = useMemo(() => ordini.map(o => ({ ...o, results: computeOrdineResults(o) })), [ordini]);
  const totalLogsOrdini = useMemo(() => ordiniResults.reduce((s, o) => s + o.results.logsDecimali, 0), [ordiniResults]);
  ...
  const results = useMemo(() => {
    const {
      tirataRiferimento, taglioRiferimento, fondello,
      lunghezzaBarra, numeroBarre, numeroLuci, pesoMTL,
      scartoTesta, scartoCoda, modalitaManuale, taglioManuale, abilitaGiunta,
      billetteEstruse
    } = inputs;
    ...
    const logsDecimali = billettePerLogEffettive > 0 ? billetteTotaliNecessarie / billettePerLogEffettive : 0;
    ...
    const bpbCorta = (tirataCorta > (scartoTesta + scartoCoda + lunghezzaBarra))
      ...
      : 0;
    const isCortaRecuperabile = !abilitaGiunta && scartoResiduoLog >= MIN_BILLET_LEN && bpbCorta > 0 && bpbCorta < bpbEffettivo;
    ...
    const billetAdvisor = BILLET_TYPES.map(log => {
      ...
      const effectiveWaste = (!abilitaGiunta && waste >= MIN_BILLET_LEN) ? waste * 0.1 : waste;
    ...
    const logRecommendations = BILLET_TYPES.flatMap(log =>
      log.options.map(opt => {
        ...
        const optBpb = (optUtileNetto > 0 && lunghezzaBarra > 0)
        ...
        return {
          logId: log.id, logName: log.name, logLen: log.length,
          cut: optCut, pieces: opt.pieces,
          tirata: optTirata, bpb: optBpb,
          billette: optBillette, logs: optLogs,
          pesoBilletta: optPesoBill, inRange,
    ...
    const progressPercent = billetteTotaliNecessarie > 0
      ? Math.min(100, (billetteEstruse / billetteTotaliNecessarie) * 100)
      : 0;
    const logsConsumati = billettePerLogEffettive > 0
    ...
    const metriRimanentiTracking = (logsRimanenti - logsRimanentiInteri) * logLen;
    const isCompletato = billetteEstruse >= billetteTotaliNecessarie && billetteTotaliNecessarie > 0;

    const warnings: string[] = [];
    const errors: string[] = [];
    if (cut <= 0) errors.push("Taglio nullo");
    if (bpbEffettivo <= 0) errors.push("Tirata proiettata insufficiente");
    if (cut > 0 && cut < MIN_BILLETTA) warnings.push(`Billetta ${cut}mm sotto il minimo (${MIN_BILLETTA}mm)`);
    if (cut > MAX_BILLETTA) warnings.push(`Billetta ${cut}mm sopra il massimo (${MAX_BILLETTA}mm)`);
    if (tirataProiettata > 0 && tirataProiettata < MIN_TIRATA) warnings.push(`Tirata ${tirataProiettata.toFixed(0)}mm sotto il minimo (${MIN_TIRATA}mm)`);
    if (tirataProiettata > MAX_TIRATA) warnings.push(`Tirata ${tirataProiettata.toFixed(0)}mm sopra il massimo (${MAX_TIRATA}mm)`);
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col font-sans selection:bg-emerald-500/30">
      <header className="bg-[#0f172a] border-b border-emerald-500/30 p-4 flex justify-between items-center shadow-xl z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Factory className={`w-4 h-4 ${currentView === 'calcoli' ? 'text-emerald-400' : 'text-slate-500'}`} />
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${currentView === 'calcoli' ? 'text-emerald-400' : 'text-slate-400'}`}>
              Home
            </span>
          </button>
          <button
            onClick={() => setCurrentView(v => v === 'gestione-log' ? 'calcoli' : 'gestione-log')}
          ...
          >
            <Flame className={`w-4 h-4 ${currentView === 'gestione-log' ? 'text-orange-400' : 'text-slate-500'}`} />
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${currentView === 'gestione-log' ? 'text-orange-400' : 'text-slate-400'}`}>
              Log
            </span>
            {(logNelForno.some(v => v > 0) || logCaricatore.some(v => v > 0)) && (
              <span className="text-[9px] font-black text-orange-400 bg-orange-500/20 px-1.5 py-0.5 rounded-full">
                {logNelForno.filter(v => v > 0).length + logCaricatore.filter(v => v > 0).length}
              </span>
            )}
          </button>
          <button
            onClick={() => setCurrentView(v => v === 'ordini' ? 'calcoli' : 'ordini')}
          ...
          >
            <ClipboardList className={`w-4 h-4 ${currentView === 'ordini' ? 'text-violet-400' : 'text-slate-500'}`} />
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${currentView === 'ordini' ? 'text-violet-400' : 'text-slate-400'}`}>
              Ordini
            </span>
            {ordini.length > 0 && (
          ...
          <button
            onClick={() => setCurrentView(v => v === 'archivio' ? 'calcoli' : 'archivio')}
          ...
          >
            <Box className={`w-4 h-4 ${currentView === 'archivio' ? 'text-blue-400' : 'text-slate-500'}`} />
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${currentView === 'archivio' ? 'text-blue-400' : 'text-slate-400'}`}>
              Archivio DB
            </span>
            {dbArticoli.length > 0 && (
          ...
          <button
            onClick={() => setCurrentView(v => v === 'report' ? 'calcoli' : 'report')}
          ...
          >
            <FileText className={`w-4 h-4 ${currentView === 'report' ? 'text-rose-400' : 'text-slate-500'}`} />
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${currentView === 'report' ? 'text-rose-400' : 'text-slate-400'}`}>
              Report
            </span>
          </button>
        </div>
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <ResultCard
                  ...
                  value={results.numCorte > 0 ? `${results.billetteTotaliNecessarie} + ${results.numCorte} corte` : results.billetteTotaliNecessarie.toString()}
                  unit="PZ"
                  sub={inputs.abilitaGiunta
                ...
                />
                <ResultCard
                  label={results.bestRecommendation ? 'Log Raccomandato' : 'Advisor Log'}
                  value={results.bestRecommendation ? `${results.bestRecommendation.logName}` : results.billetAdvisor.name}
                  unit="LOG"
                  sub={results.bestRecommendation
                  ...
                  highlight={results.bestRecommendation?.isCurrentLog || results.billetAdvisor.id === inputs.billettoneId}
                  color={results.bestRecommendation?.isCurrentCut ? 'text-emerald-400' : 'text-amber-400'}
                />
                <ResultCard
                  label="Carico Log"
                  value={results.metriRimanenti > 0 ? `${results.logsInteri} LOG + ${results.metriRimanenti.toFixed(0)}mm` : `${results.logsInteri} LOG`}
                  unit="LOGS"
                  sub={inputs.abilitaGiunta ? `${(results.totalMetriLineari / 1000).toFixed(1)}m lineari · ${results.logsDecimali.toFixed(2)} logs` : `Totale: ${results.logsDecimali.toFixed(2)} logs`}
              ...
              {results.warnings.length > 0 && (
                <div className="space-y-2 animate-in slide-in-from-top-2">
                  {results.warnings.map((w, i) => (
                    <div key={i} className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                      <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
                      <span className="text-[10px] font-bold text-amber-300 uppercase">{w}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {inputs.billetteEstruse > 0 && (
                <div className={`border rounded-xl p-4 space-y-3 animate-in slide-in-from-bottom-2 transition-all duration-500 ${results.isCompletato
                  ? 'bg-emerald-500/[0.06] border-emerald-500/30'
                  : 'bg-cyan-500/[0.04] border-cyan-500/20'
                  }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`p-1.5 rounded-lg ${results.isCompletato
                      ...
                      <div>
                        <h4 className={`text-[9px] font-black uppercase tracking-widest ${results.isCompletato ? 'text-emerald-400' : 'text-cyan-400'
                          }`}>
                          {results.isCompletato ? 'Produzione Completata' : 'Avanzamento Produzione'}
                        </h4>
                        <p className="text-[8px] font-bold text-slate-500 uppercase">
                          {inputs.billetteEstruse} / {results.billetteTotaliNecessarie} billette
                        </p>
                      </div>
                    </div>
                    <span className={`text-lg font-black mono ${results.isCompletato ? 'text-emerald-400' : 'text-cyan-400'
                      }`}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/50">
                      <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Billette Rimaste</span>
                      <span className={`text-sm font-black mono ${results.billetteRimanenti === 0 ? 'text-emerald-400' : 'text-white'
                        }`}>{results.billetteRimanenti}</span>
                    </div>
                    <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/50">
                      <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Barre Prodotte</span>
                      <span className="text-sm font-black text-cyan-400 mono">{results.barreEstruseTotali}</span>
                    </div>
                    <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/50">
                      <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Barre Rimaste</span>
                      <span className={`text-sm font-black mono ${results.barreRimanenti === 0 ? 'text-emerald-400' : 'text-amber-400'
                        }`}>{results.barreRimanenti}</span>
                    </div>
                    <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/50">
                      <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Logs Rimasti</span>
                      <span className={`text-sm font-black mono ${results.logsRimanenti <= 0 ? 'text-emerald-400' : 'text-white'
                        }`}>{results.logsRimanenti <= 0 ? '0' : (results.metriRimanentiTracking > 0 ? `${results.logsRimanentiInteri} LOG + ${results.metriRimanentiTracking.toFixed(0)}mm` : `${results.logsRimanentiInteri} LOG`)}</span>
                    </div>
                  </div>

                  {results.isCompletato && inputs.billetteEstruse > results.billetteTotaliNecessarie && (
                    <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-lg p-2">
                      <AlertTriangle className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                      <span className="text-[9px] font-bold text-amber-300">
                        Estruse {inputs.billetteEstruse - results.billetteTotaliNecessarie} billette in eccesso
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 shadow-xl">
                  <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tagli Ottimizzati — {selectedLog?.name}</h3>
                    </div>
                    {results.bestRecommendation && !results.bestRecommendation.isCurrentLog && (
                      <button
                        onClick={() => setInputs(p => ({ ...p, billettoneId: results.bestRecommendation!.logId }))}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                    {selectedLog?.options.map((opt) => {
                      ...
                      const rec = results.logRecommendations.find(r => r.isCurrentLog && r.cut === opt.length);
                      const isBest = results.bestRecommendation?.isCurrentLog && results.bestRecommendation?.cut === opt.length;
                      return (
                        <button key={opt.length} onClick={() => setSelectedCut(opt.length)} className={`group p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-1 relative ${isSelected ? 'border-emerald-500 bg-emerald-500/10'
                          : isBest ? 'border-amber-500/50 bg-amber-500/5'
                            : 'border-slate-800 bg-slate-950/50'
                          }`}>
                          {isBest && !isSelected && (
                            <span className="absolute -top-2 text-[7px] font-black text-amber-400 bg-amber-500/20 px-1.5 py-0.5 rounded-full uppercase">Best</span>
                          )}
                          <span className={`text-xl font-black mono ${isSelected ? 'text-white' : isBest ? 'text-amber-400' : 'text-slate-500'}`}>{opt.length}</span>
                          <span className="text-[9px] font-bold text-slate-600 uppercase">{opt.pieces} PZ</span>
                          {rec && <span className="text-[8px] font-bold text-slate-600 mono">{rec.bpb} barre</span>}
                          {rec && <span className={`text-[9.5px] font-black mono mt-1 px-1.5 py-0.5 rounded-sm ${isSelected ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-cyan-500/10 text-cyan-500/80 border border-cyan-500/20'}`}>TIRATA: {(rec.tirata / 1000).toFixed(1)}m</span>}
                        </button>
                      );
                    })}
                  </div>
                  {results.bestRecommendation && (
                    <div className={`mt-4 p-3 rounded-lg border flex items-start gap-3 ${results.bestRecommendation.isCurrentCut
                      ? 'bg-emerald-500/[0.06] border-emerald-500/30'
                      : 'bg-amber-500/[0.06] border-amber-500/30'
                      }`}>
                      <ThumbsUp className={`w-4 h-4 mt-0.5 shrink-0 ${results.bestRecommendation.isCurrentCut ? 'text-emerald-400' : 'text-amber-400'
                        }`} />
                      <div className="space-y-1">
                        <span className={`text-[9px] font-black uppercase ${results.bestRecommendation.isCurrentCut ? 'text-emerald-400' : 'text-amber-400'
                          }`}>
                        ...
                        <p className="text-[8px] font-bold text-slate-500 leading-relaxed">
                          Tirata {(results.bestRecommendation.tirata / 1000).toFixed(1)}m · {results.bestRecommendation.bpb} barre/bill · {Math.ceil(results.bestRecommendation.logs)} logs · {results.bestRecommendation.pesoBilletta.toFixed(1)} kg/bill
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 pt-4 relative z-10">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-6 bg-slate-950/60 p-6 rounded-2xl border border-slate-800/50">
                      ...
                      <MetricRow label="Carico Ordine" value={`${results.logsInteri} LOG`} sub="Totale logs ordine" />
                    </div>
                  </div>
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 p-6 lg:p-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-white">Gestione Log</h2>
                </div>
                <button
                  onClick={() => setCurrentView('calcoli')}
              ...
              {(() => {
                const logCaricati = logNelForno.filter(v => v > 0).length;
                const totalMmForno = logNelForno.reduce((s, v) => s + v, 0);
                const totalMmCaricatoreAll = logCaricatore.reduce((s, v) => s + v, 0);
                ...
                const mmRimanenti = Math.max(0, totalMmNeeded - totalMmCaricati);
                  ...
                  <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Log nel Forno</h3>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[9px] font-black text-slate-500 uppercase">{logCaricati} / 5 caricati</span>
                        {logCaricati > 0 && (
                          <button
                            onClick={() => { setLogNelForno([0, 0, 0, 0, 0]); setEditingSlot(null); }}
                          ...
                          {editingSlot === i ? (
                            <div className="h-20 rounded-xl border-2 border-orange-500/50 bg-slate-950 flex flex-col items-center justify-center p-2">
                              <input
                                ...
                                onBlur={(e) => {
                                  if (e.currentTarget.dataset.navigating) return;
                                  const v = Math.min(8000, Math.max(0, parseInt(e.target.value) || 0));
                                  setLogNelForno(prev => { const n = [...prev]; n[i] = v; return n; });
                                  setEditingSlot(null);
                                }}
                                onKeyDown={(e) => {
                                  if (e.key === 'Tab' || e.key === 'Enter') {
                                    e.preventDefault();
                                    const v = Math.min(8000, Math.max(0, parseInt((e.target as HTMLInputElement).value) || 0));
                                    setLogNelForno(prev => { const n = [...prev]; n[i] = v; return n; });
                                    const next = e.shiftKey ? i - 1 : i + 1;
                                    if (next >= 0 && next < 5) {
                                      (e.currentTarget as HTMLInputElement).dataset.navigating = '1';
                                      setEditingSlot(next);
                                    } else {
                                      setEditingSlot(null);
                                    }
                              ...
                              />
                              <span className="text-[7px] font-black text-orange-500/60 uppercase">Tab ➜ prossimo</span>
                            </div>
                          ) : (
                            <button
                            ...
                            >
                              {val > 0 && (
                                <span
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setLogNelForno(prev => { const n = [...prev]; n[i] = 0; return n; });
                                  }}
                                  className="absolute top-1 right-1.5 text-[9px] font-black text-slate-600 hover:text-red-400 cursor-pointer transition-colors"
                                >
                                  ✕
                                </span>
                              )}
                              {val > 0 ? (
                                <>
                                  <span className="text-lg font-black text-orange-400 mono">{val}</span>
                                  <span className="text-[7px] font-black text-orange-500/60 uppercase">mm</span>
                                </>
                              ) : (
                                <>
                                  <Factory className="w-5 h-5 text-slate-700" />
                                  <span className="text-[8px] font-black uppercase text-slate-700">Vuoto</span>
                                </>
                              )}
                            </button>
                          )}
                    ...
                    {logCaricati > 0 && (
                      <div className="grid grid-cols-3 gap-3">
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Metri nel Forno</span>
                          <span className="text-lg font-black text-orange-400 mono">{(totalMmForno / 1000).toFixed(1)}m</span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Carico Necessario</span>
                          <span className="text-lg font-black text-white mono">
                            {results.metriRimanenti > 0 ? `${results.logsInteri} LOG + ${results.metriRimanenti.toFixed(0)}mm` : `${results.logsInteri} LOG`}
                          </span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Logs Mancanti</span>
                          <span className={`text-lg font-black mono ${tuttiCaricati ? 'text-emerald-400' : 'text-amber-400'}`}>
                            {tuttiCaricati ? (mmSurplus > 0 ? `+${mmSurplus.toFixed(0)}mm` : '0') : (mmMancFrazione > 0 ? `${logsMancInteri} LOG + ${mmMancFrazione.toFixed(0)}mm` : `${logsMancInteri} LOG`)}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {tuttiCaricati && (
                      <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-3">
                        ...
                        <span className="text-[9px] font-bold text-emerald-300 uppercase">Tutti i log sono caricati!</span>
                      </div>
                    )}
                  </div>
                );
              ...
              {(() => {
                const logCaricatiC = logCaricatore.filter(v => v > 0).length;
                const totalMmCaricatore = logCaricatore.reduce((s, v) => s + v, 0);
                  ...
                  <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Log Caricatore</h3>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[9px] font-black text-slate-500 uppercase">{logCaricatiC} / 12 caricati</span>
                        {logCaricatiC > 0 && (
                          <button
                            onClick={() => { setLogCaricatore(Array(12).fill(0)); setEditingSlotCaricatore(null); }}
                            className="flex items-center gap-1 text-[8px] font-black uppercase text-red-400/60 hover:text-red-400 transition-colors"
                          >
                            <RotateCcw className="w-3 h-3" /> Reset
                          </button>
                            ...
                            <div className="h-16 rounded-lg border-2 border-blue-500/50 bg-slate-950 flex flex-col items-center justify-center p-1.5">
                              <input
                                ...
                                onBlur={(e) => {
                                  if (e.currentTarget.dataset.navigating) return;
                                  const v = Math.min(8000, Math.max(0, parseInt(e.target.value) || 0));
                                  setLogCaricatore(prev => { const n = [...prev]; n[i] = v; return n; });
                                  setEditingSlotCaricatore(null);
                                }}
                                onKeyDown={(e) => {
                                  if (e.key === 'Tab' || e.key === 'Enter') {
                              ...
                              />
                              <span className="text-[6px] font-black text-blue-500/60 uppercase">Tab ➜</span>
                            </div>
                          ) : (
                            <button
                            ...
                            >
                              {val > 0 && (
                                <span
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setLogCaricatore(prev => { const n = [...prev]; n[i] = 0; return n; });
                                  }}
                                  className="absolute top-0.5 right-1 text-[8px] font-black text-slate-600 hover:text-red-400 cursor-pointer transition-colors"
                                >
                                  ✕
                                </span>
                              )}
                              {val > 0 ? (
                                <>
                                  <span className="text-sm font-black text-blue-400 mono">{val}</span>
                                  <span className="text-[6px] font-black text-blue-500/60 uppercase">mm</span>
                                </>
                              ) : (
                                <span className="text-[7px] font-black uppercase text-slate-700">Vuoto</span>
                              )}
                            </button>
                          )}
                        </div>
                    ...
                    {logCaricatiC > 0 && (
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Log Caricatore</span>
                          <span className="text-lg font-black text-blue-400 mono">{logCaricatiC}</span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Metri Caricatore</span>
                          <span className="text-lg font-black text-blue-400 mono">{(totalMmCaricatore / 1000).toFixed(1)}m</span>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500 p-6 lg:p-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-white">Gestione Ordini</h2>
                  {ordini.length > 0 && (
              ...
              <div className={`flex items-center gap-4 bg-slate-900/50 border rounded-xl p-4 transition-all duration-300 ${barcodeFeedback === 'success' ? 'border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : barcodeFeedback === 'error' ? 'border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'border-slate-800 focus-within:border-violet-500/50'}`}>
                <div className={`p-2 rounded-lg ${barcodeFeedback === 'success' ? 'bg-emerald-500/20 text-emerald-400' : barcodeFeedback === 'error' ? 'bg-red-500/20 text-red-400' : 'bg-violet-500/20 text-violet-400'}`}>
                  <ScanLine className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Scanner Ordine cartaceo</span>
                  <input
                    type="text"
                    value={barcodeInput}
                ...
                {barcodeFeedback === 'success' && <span className="text-[10px] font-black text-emerald-400 uppercase bg-emerald-500/10 px-2 py-1 rounded">Inserito ✓</span>}
                {barcodeFeedback === 'error' && <span className="text-[10px] font-black text-red-400 uppercase bg-red-500/10 px-2 py-1 rounded">Errore formato ✗</span>}
              </div>

              {ordini.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                  <ClipboardList className="w-12 h-12 text-violet-500/50 mb-4" />
                  <p className="text-[11px] font-black uppercase text-slate-500 tracking-wider">Nessun ordine inserito</p>
                  <p className="text-[9px] text-slate-600 mt-1">Clicca "Aggiungi Ordine" per iniziare</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {ordiniResults.map((ord) => {
                    ...
                    const r = ord.results;
                    return (
                      <div key={ord.id} className={`bg-slate-900/30 border rounded-xl transition-all duration-300 overflow-hidden ${isExpanded ? 'border-violet-500/40' : 'border-slate-800'
                        }`}>
                        <div
                          className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-800/30 transition-colors"
                          onClick={() => setExpandedOrdine(isExpanded ? null : ord.id)}
                        >
                          <div className="flex items-center gap-3">
                            {isExpanded ? <ChevronDown className="w-4 h-4 text-violet-400" /> : <ChevronRight className="w-4 h-4 text-slate-600" />}
                            <div>
                              <span className="text-sm font-black text-white mono flex items-center gap-2">
                                {ord.articolo || 'Nuovo Articolo'}
                                {ord.numeroBarra && <span className="text-slate-500">/{ord.numeroBarra}</span>}
                                {ord.numeroBarra && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setActiveDieModal({ articolo: ord.articolo, numeroBarra: ord.numeroBarra! });
                                    }}
                              ...
                              {ord.lega && <span className="text-[11px] font-bold text-amber-400/70 ml-2">{ord.lega}</span>}
                              <div className="flex items-center gap-3 mt-1">
                                <span className="text-[10px] font-bold text-slate-500">{r.bpb} barre/bill{r.giuntaExtra > 0 ? ` (+${r.giuntaExtra} comp.)` : ''}</span>
                                <span className="text-[10px] font-bold text-slate-500">{r.billette} billette</span>
                                <span className="text-[10px] font-bold text-violet-400">{Math.ceil(r.logsDecimali)} logs ({r.logName})</span>
                                <span className="text-[10px] font-bold text-slate-500">{ord.numeroBarre} barre (+10% = {Math.floor(ord.numeroBarre * 1.1)})</span>
                                {ord.abilitaGiunta && <span className="text-[10px] font-bold text-cyan-400">COMP.</span>}
                                {r.isAutoOptimized && <span className="text-[10px] font-bold text-emerald-400">AUTO-CUT: {r.cut}mm</span>}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setOrdini(prev => prev.filter(o => o.id !== ord.id));
                                if (expandedOrdine === ord.id) setExpandedOrdine(null);
                              }}
                        ...
                        {isExpanded && (
                          <div className="px-4 pb-4 space-y-4 border-t border-slate-800/50 pt-4 animate-in slide-in-from-top-2">
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                              <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase block mb-1">N° Articolo</label>
                                <input
                                  value={ord.articolo}
                                  onChange={(e) => setOrdini(prev => prev.map(o => o.id === ord.id ? { ...o, articolo: e.target.value } : o))}
                              ...
                              <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase block mb-1">Lega</label>
                                <input
                                  value={ord.lega}
                                  onChange={(e) => setOrdini(prev => prev.map(o => o.id === ord.id ? { ...o, lega: e.target.value } : o))}
                                  placeholder="Es. 6060"
                              ...
                              <div className="flex-1 min-w-[120px]">
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1 block">N° Barra</label>
                                <input
                                  type="text"
                                  value={ord.numeroBarra || ''}
                                  placeholder="es. 3A"
                                  onChange={(e) => setOrdini(prev => prev.map(o => o.id === ord.id ? { ...o, numeroBarra: e.target.value.toUpperCase() } : o))}
                              ...
                              {[
                                { label: 'Tirata Rif. (mm)', key: 'tirataRiferimento', optional: true },
                                { label: 'Taglio Rif. (mm)', key: 'taglioRiferimento', optional: true },
                                { label: 'Fondello (mm)', key: 'fondello' },
                                { label: 'Lungh. Barra (mm)', key: 'lunghezzaBarra' },
                                { label: `N° Barre ${ord.numeroBarre > 0 ? `(+10% = ${Math.floor(ord.numeroBarre * 1.1)})` : ''}`, key: 'numeroBarre' },
                                { label: 'N° Luci', key: 'numeroLuci' },
                                { label: 'Peso MTL (g/m)', key: 'pesoMTL' },
                                { label: 'Scarto Testa (mm)', key: 'scartoTesta' },
                                { label: 'Scarto Coda (mm)', key: 'scartoCoda' },
                                ...
                                <IndustrialInput
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setOrdini(prev => prev.map(o => o.id === ord.id ? { ...o, [key]: parseFloat(val) || 0 } : o));
                                  }}
                                  optional={optional}
                                  compact
                                />
                              ...
                              <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase block mb-1">Log</label>
                                <select
                                  value={ord.billettoneId}
                                  onChange={(e) => setOrdini(prev => prev.map(o => o.id === ord.id ? { ...o, billettoneId: e.target.value } : o))}
                                ...
                                >
                                  {BILLET_TYPES.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                                </select>
                              </div>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                              <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-800/50 text-center">
                                <span className="text-[9px] font-black text-slate-500 uppercase block mb-0.5">Tirata</span>
                                <span className="text-base font-black text-white mono">{(r.tirata / 1000).toFixed(1)}m</span>
                              </div>
                              <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-800/50 text-center">
                                <span className="text-[9px] font-black text-slate-500 uppercase block mb-0.5">Barre/Bill</span>
                                <span className="text-base font-black text-emerald-400 mono">{r.bpb}{r.giuntaExtra > 0 ? ` +${r.giuntaExtra}` : ''}</span>
                              </div>
                              <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-800/50 text-center">
                                <span className="text-[9px] font-black text-slate-500 uppercase block mb-0.5">Billette</span>
                                <span className="text-base font-black text-white mono">{r.billette}</span>
                              </div>
                              <div className="p-3 bg-violet-500/10 rounded-lg border border-violet-500/30 text-center">
                                <span className="text-[9px] font-black text-violet-400 uppercase block mb-0.5">Logs</span>
                                <span className="text-base font-black text-violet-400 mono">{r.logsDecimali.toFixed(2)}</span>
                              </div>
                            </div>

                            {r.recommendation && (r.recommendation.logId !== r.logId || r.recommendation.cut !== r.cut) && (
                              <div className="bg-emerald-500/[0.06] border border-emerald-500/30 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                  <span className="text-[9px] font-black text-emerald-400 uppercase tracking-wider block mb-1">Raccomandazione</span>
                                  <p className="text-[10px] text-emerald-300/80">
                                    Usa <b>{r.recommendation.logName}</b> con taglio <b>{r.recommendation.cut}mm</b> ({r.recommendation.pieces} bpb) → <b>{Math.ceil(r.recommendation.logsNeeded)} logs</b> (scarto {r.recommendation.scarto}mm)
                                  </p>
                                  <button
                                    onClick={() => setOrdini(prev => prev.map(o => o.id === ord.id
                                      ? { ...o, billettoneId: r.recommendation!.logId, taglioManuale: r.recommendation!.cut }
                                      : o
                                    ))}
              ...
              {ordini.length > 0 && (
                <div className="bg-violet-500/[0.06] border border-violet-500/30 rounded-2xl p-6 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[11px] font-black uppercase tracking-widest text-violet-400">Riepilogo Ordini</h3>
                  </div>
                  {(() => {
                    const totalMmForno = logNelForno.reduce((s, v) => s + v, 0);
                    const totalMmCaricatoreAll = logCaricatore.reduce((s, v) => s + v, 0);
                    const totalMmDisponibili = totalMmForno + totalMmCaricatoreAll;
                    // Use average log length across orders for conversion
                    const avgLogLen = ordiniResults.length > 0
                      ? ordiniResults.reduce((s, o) => s + o.results.logLen, 0) / ordiniResults.length
                      : 8000;
                    const totalMmNeeded = totalLogsOrdini * avgLogLen;
                    const mmMancanti = Math.max(0, totalMmNeeded - totalMmDisponibili);
                    const logsMancInteri = Math.floor(mmMancanti / avgLogLen);
                    const mmFrazione = mmMancanti - (logsMancInteri * avgLogLen);
                    const tuttiCaricati = totalMmDisponibili >= totalMmNeeded && ordini.length > 0;

                    return (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Logs Totali</span>
                          <span className="text-lg font-black text-violet-400 mono">{totalLogsOrdini.toFixed(2)}</span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Forno + Caricatore</span>
                          <span className="text-lg font-black text-orange-400 mono">{((totalMmDisponibili) / 1000).toFixed(1)}m</span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Logs Mancanti</span>
                          <span className={`text-lg font-black mono ${tuttiCaricati ? 'text-emerald-400' : 'text-amber-400'}`}>
                            {tuttiCaricati ? '0' : (mmFrazione > 0 ? `${logsMancInteri} LOG + ${mmFrazione.toFixed(0)}mm` : `${logsMancInteri} LOG`)}
                          </span>
                        </div>
                        <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/50">
                          <span className="text-[7px] font-black text-slate-500 uppercase block mb-0.5">Billette Totali</span>
                          <span className="text-lg font-black text-white mono">
                            {(() => {
                              const bTot = ordiniResults.reduce((s, o) => s + o.results.billette, 0);
                              const cTot = ordiniResults.reduce((s, o) => {
                                const r = o.results;
                                const remainderLen = r.logLen > 0 && r.cut > 0 ? r.logLen - (Math.floor(r.logLen / r.cut) * r.cut) : 0;
                                const uCorta = Math.max(0, remainderLen - (o.fondello || 0));
                                const tirCorta = uCorta * r.ratio;
                                const bpbCortaVal = (tirCorta > (o.scartoTesta + o.scartoCoda + o.lunghezzaBarra))
                                  ? Math.floor((tirCorta - o.scartoTesta - o.scartoCoda) / o.lunghezzaBarra) * o.numeroLuci
                                  : 0;
                                // Corta solo se produce MENO barre della billetta standard
                                const isCorta = !inputs.abilitaGiunta && remainderLen >= MIN_BILLET_LEN && bpbCortaVal > 0 && bpbCortaVal < r.bpb;
                                return s + (isCorta ? Math.floor(r.logsDecimali) : 0);
                              }, 0);
                              return cTot > 0 ? `${bTot} / ${cTot} Corte` : bTot.toString();
                            })()}
                          </span>
                        </div>
                      </div>
                  ...
                  {(() => {
                    ...
                    const avgLogLen = ordiniResults.length > 0
                      ...
                      : 8000;
                    const totalMmNeeded = totalLogsOrdini * avgLogLen;
                    if (totalMmForno + totalMmCaricatoreAll >= totalMmNeeded && ordini.length > 0) {
                      return (
                        <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-3">
                          <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                          <span className="text-[9px] font-bold text-emerald-300 uppercase">Tutti i log per gli ordini sono coperti!</span>
                        </div>
                      );
                    }
                    return null;
            ...
            <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500 p-6 lg:p-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-white">Archivio Database</h2>
                  {dbArticoli.length > 0 && (
              ...
              {dbArticoli.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                  ...
                  <p className="text-[11px] font-black uppercase text-slate-500 tracking-wider">Database Vuoto</p>
                  <p className="text-[9px] text-slate-600 mt-1">Salva un ordine dalla sezione "Gestione Ordini" per aggiungerlo qui.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {dbArticoli.filter(art => art.articolo.toLowerCase().includes(dbSearchQuery.toLowerCase())).map((art) => (
                    <div key={art.id} className="bg-slate-900/30 border border-slate-800 rounded-xl overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-slate-800/30 transition-colors">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <span className="text-sm font-black text-white mono flex items-center gap-2">
                            {art.articolo}
                            {art.numeroBarra && <span className="text-slate-500">/{art.numeroBarra}</span>}
                            {art.numeroBarra && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setActiveDieModal({ articolo: art.articolo, numeroBarra: art.numeroBarra! });
                                }}
                                className="flex items-center justify-center bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 rounded px-1.5 py-0.5 transition-colors"
                                title="Storico Matrice"
                              >
                          ...
                          <span className="text-[9px] font-bold text-amber-400/70 border border-amber-500/20 px-1.5 py-0.5 rounded-md">{art.lega}</span>
                          {art.abilitaGiunta && <span className="text-[8px] font-bold text-cyan-400 border border-cyan-500/20 px-1.5 py-0.5 rounded-md">COMPENSATO</span>}
                        </div>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5">
                          <span className="text-[9px] font-bold text-slate-500">Tirata Rif: <b className="text-emerald-400/80">{art.tirataRiferimento}mm</b></span>
                          <span className="text-[9px] font-bold text-slate-500">Taglio Rif: <b className="text-amber-400/80">{art.taglioRiferimento}mm</b></span>
                          <span className="text-[9px] font-bold text-slate-500">L. Barra: <b>{art.lunghezzaBarra}mm</b></span>
                          <span className="text-[9px] font-bold text-slate-500">Peso: <b>{art.pesoMTL} kg/m</b></span>
                        </div>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                          <span className="text-[8px] font-bold text-slate-600 uppercase">Fondello: {art.fondello}mm</span>
                          <span className="text-[8px] font-bold text-slate-600 uppercase">Scarti: {art.scartoTesta} / {art.scartoCoda}</span>
                          <span className="text-[8px] font-bold text-slate-600 uppercase">Log: {BILLET_TYPES.find(b => b.id === art.billettoneId)?.name || art.billettoneId}</span>
                        </div>
                      </div>
            ...
            <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500 p-6 lg:p-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-white">Report Produzione PDF</h2>
                </div>
                <button
                  onClick={() => setCurrentView('calcoli')}
                  className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 bg-slate-800/50 border border-slate-700 px-4 py-2 rounded-lg hover:text-white hover:border-slate-600 transition-colors"
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 space-y-5">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Filtri Report</h3>

                <div className="flex gap-2 mb-4">
                  <button onClick={() => setReportFilterMode('day')} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-colors ${reportFilterMode === 'day' ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>Per Giorno</button>
                  <button onClick={() => setReportFilterMode('month')} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-colors ${reportFilterMode === 'month' ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>Per Mese</button>
                </div>

                {reportFilterMode === 'day' ? (
                  <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                    <label className="text-[8px] font-black uppercase text-slate-500 mb-1 block">Data Specifica</label>
                    <input
                      type="date"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                      <label className="text-[8px] font-black uppercase text-slate-500 mb-1 block">Mese</label>
                      <select value={reportFilterMonth} onChange={e => setReportFilterMonth(parseInt(e.target.value))} className="w-full bg-transparent text-sm font-black text-white mono outline-none">
                    ...
                    <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                      <label className="text-[8px] font-black uppercase text-slate-500 mb-1 block">Anno</label>
                      <input type="number" value={reportFilterYear} onChange={e => setReportFilterYear(parseInt(e.target.value) || 2026)} className="w-full bg-transparent text-sm font-black text-white mono outline-none" />
                    </div>
                  </div>
                )}

                <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                  <label className="text-[8px] font-black uppercase text-slate-500 mb-2 block">Turno</label>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={t.val}
                        onClick={() => setReportFilterTurno(t.val as 0 | 1 | 2 | 3)}
                      ...
                      >
                        <span className="block font-black uppercase">{t.label}</span>
                        {t.time && <span className="text-[8px] opacity-70 font-normal">{t.time}</span>}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {(() => {
                const filtered = dieHistory.filter(h => {
                  ...
                  if (reportFilterMode === 'day' && reportFilterDate) {
                    ...
                    if (d.getFullYear() !== y || d.getMonth() + 1 !== m || d.getDate() !== dd) return false;
                  } else if (reportFilterMode === 'month') {
                    if (d.getFullYear() !== reportFilterYear || d.getMonth() + 1 !== reportFilterMonth) return false;
                  }
                  if (reportFilterTurno !== 0 && h.turno !== reportFilterTurno) return false;
                  return true;
                });
                const totalB = filtered.reduce((s, e) => s + e.billetteEstruse, 0);
                const uniqueArt = [...new Set(filtered.map(e => e.articolo))];
                const avgS = filtered.filter(e => e.scartoReale != null).length > 0
                  ? (filtered.filter(e => e.scartoReale != null).reduce((s, e) => s + (e.scartoReale || 0), 0) / filtered.filter(e => e.scartoReale != null).length).toFixed(0)
                  : '—';
                  ...
                  <>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Billette Totali</span>
                        <span className="text-2xl font-black text-emerald-400 mono">{totalB}</span>
                      </div>
                      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Sessioni</span>
                        <span className="text-2xl font-black text-indigo-400 mono">{filtered.length}</span>
                      </div>
                      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Scarto Medio</span>
                        <span className="text-2xl font-black text-amber-400 mono">{avgS}<span className="text-sm text-slate-500 ml-1">mm</span></span>
                      </div>
                      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Articoli</span>
                        <span className="text-2xl font-black text-cyan-400 mono">{uniqueArt.length}</span>
                      </div>
                    </div>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (reportFilterTurno > 0) filterLabel += ` — ${reportFilterTurno}° Turno`;
                          ...
                          formData.append('Instructions', new Blob([instructions], { type: 'application/json' }), 'instructions.json');
                          ...
                          if (!res.ok) {
                            ...
                            console.error('dpdf.io error:', errText);
                    ...
                    >
                      {isGeneratingPdf
                        ? <><span className="animate-spin">⏳</span> Generazione PDF in corso...</>
                        : <><Download className="w-4 h-4" /> Scarica Report PDF ({filtered.length} sessioni)</>
                      }
                    </button>
                  </>
                );
                ...
                <div className="bg-slate-900/30 border border-slate-800 rounded-xl overflow-hidden">
                  <div className="p-3 border-b border-slate-800/50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Storico Ordini Completati ({completedOrders.length})</span>
                    </div>
                  </div>
                  <div className="max-h-72 overflow-y-auto">
                    {completedOrders.map((co, i) => (
                      <div key={co.id} className={`flex items-center justify-between px-4 py-3 ${i % 2 === 0 ? 'bg-slate-950/30' : ''}`}>
                        <div className="flex items-center gap-3">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500/60" />
                          <div>
                            <span className="text-xs font-black text-white mono">{co.articolo}{co.numeroBarra ? ` / ${co.numeroBarra}` : ''}</span>
                            {co.lega && <span className="text-[10px] text-amber-400/70 ml-2">{co.lega}</span>}
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[9px] text-slate-500">{co.billette} bill.</span>
                              <span className="text-[9px] text-slate-500">{co.logsDecimali.toFixed(1)} logs ({co.logName})</span>
                              <span className="text-[9px] text-slate-500">cut {co.cut}mm</span>
                            </div>
                          </div>
                        </div>
                        <span className="text-[9px] text-slate-600 shrink-0">{co.completedAt}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <AnimatePresence>
          {activeDieModal && (
            <DieHistoryModal
              ...
              history={dieHistory.filter(h => h.articolo === activeDieModal.articolo && h.numeroBarra === activeDieModal.numeroBarra)}
              onClose={() => setActiveDieModal(null)}
              onSave={async (billette, scarto, turno, note, operatore) => {
                await handleSaveDieHistory(activeDieModal.articolo, activeDieModal.numeroBarra, billette, scarto, turno, note, operatore);
              }}
            />
          )}
        </AnimatePresence>
